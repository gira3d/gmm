...
typedef gmm_utils::GMM4Base<float> GMM;
typedef SKLearn<float,4,-1,-1> SK;
            ...
            int nrhs, const mxArray *prhs[])
{
  ...
  if (!strcmp("new", cmd))
  {
    ...
    if (X.rows() != 4)
    {
      mexErrMsgTxt("input data must be 4xn");
      return;
    }
  ...
  if (!strcmp("create_mex_object", cmd))
  {
    ...
    if (covs.rows() != 16)
    {
      mexErrMsgTxt("input data must be 16xn");
      return;
    }
  ...
  if (!strcmp("fast_sample", cmd))
  {
    ...
    Eigen::Matrix<float, 4, -1> samples = Eigen::Matrix<float,4,-1>::Zero(4, n);
  ...
  if (!strcmp("regress", cmd))
  {
    if (nrhs != 3)
    {
      mexErrMsgTxt("set_support_size: inputs required: <\"regress\", gmm.objectHandle, X>");
      return;
    }
    ...
    if (X.rows() != 3) { mexErrMsgTxt("set_support_size: inputs required: <\"regress\", gmm.objectHandle, X>"); return; }

    Eigen::Matrix<float,-1,1> m,v;
    gmm->regress(X,m,v);
    cu::eigenToMATLAB<float>(m, 0, plhs);
    cu::eigenToMATLAB<float>(v, 1, plhs);

    return;
  }

  if (!strcmp("conditional_weights", cmd))
  {
    if (nrhs != 3)
    {
      mexErrMsgTxt("conditional_weights: inputs required: <\"conditionalWeights\", gmm.objectHandle, X>");
      return;
    }
    ...
    Eigen::MatrixXf X;
    cu::matlabToEigen(prhs, 2, X);
    std::cerr << "Size of X: " << X.rows() << X.cols() << std::endl;
    std::cerr << "X: " << X << std::endl;

    if (X.rows() != 3)
      X.transposeInPlace();

    if (X.rows() != 3) { mexErrMsgTxt("condtional_weights: inputs required: <\"conditionalWeights\", gmm.objectHandle, X>"); return; }

    Eigen::Matrix<float,-1,-1> wj;
    gmm->conditionalWeights(X,wj);
    cu::eigenToMATLAB<float>(wj, 0, plhs);

    return;
  }

  if (!strcmp("conditional_meanvar", cmd))
  {
    if (nrhs != 3)
    {
      mexErrMsgTxt("conditional_meanvar: inputs required: <\"conditionalMeanVar\", gmm.objectHandle, X>");
      return;
    }
    ...
    Eigen::Matrix<float,-1,-1> mj, s2j;
    gmm->conditionalMeanVar(X,mj,s2j);
    cu::eigenToMATLAB<float>(mj, 0, plhs);
    cu::eigenToMATLAB<float>(s2j, 1, plhs);

    return;
  }

  if (!strcmp("get3dgmm", cmd))
  {
    if (nrhs != 2)
    {
      mexErrMsgTxt("get3dgmm: inputs required: <\"get3dgmm\", gmm.objectHandle>");
      return;
    }
    ...
    GMM* gmm = convertMat2Ptr<GMM>(prhs[1]);
    std::cerr << "1" << std::endl;
    gmm_utils::GMM3Base<float> gmm3 = gmm->get3DGMM();
    std::cerr << "2" << std::endl;
    plhs[0] = convertPtr2Mat<gmm_utils::GMM3Base<float>>(new gmm_utils::GMM3Base<float>(gmm3));
    std::cerr << "3" << std::endl;
